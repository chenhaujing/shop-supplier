      ...
      <!-- 精简代码结构 -->
       <!--  <div class="orderList_nav">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item v-if="isAdmin"><i class="el-icon-date"></i> 供应商管理</el-breadcrumb-item>
                <el-breadcrumb-item v-if="isAdmin">
                    <router-link :to="{path: '/supplierInfoList'}" >
                        供应商列表
                    </router-link>
                </el-breadcrumb-item>
                 <el-breadcrumb-item v-if="!isAdmin"><i class="el-icon-date"></i> 订单管理</el-breadcrumb-item>
                <el-breadcrumb-item>
        ...
        <div class="orderList_searchWrap">
            ...
            <div class="orderList_search row clearMargin clearPadding publicHeader">
                <div class="col-xs-4 clearPadding date_inputs">
                    ...
                    <span class="col-xs-1" style="text-align: center;line-height: 36px;">~</span>
                    <div class="col-xs-4 dateInput">
                        <el-date-picker
                ...
                <div class="col-xs-2 clearPadding">
                    <button class="el-button el-button--info" @click="advancedSearch">高级搜索</button>
                    <button type="button" class="btn-info el-button" @click="dialogVisible=true"
                            style="margin:-2px 0;" v-has:orderList_exportOrder>导出
                    </button>
                    <!-- 导出模态框 -->
                ...
                <div class="col-xs-1">
                    <el-button type="default" class="glyphicon glyphicon-list active" role="presentation" @click="slideToggle" v-has:orderList_confirmDisplay></el-button>
                </div>
            </div>
            <!--高级搜索部分 刘述修改不要覆盖-->
            ...
            <div class="orderList_tableWrap">
                <div class="table-responsive orderList_tableWrap orderList_mainTableParent">
                    <table class="table table-bordered table-hover orderList_mainTable text-center" id="orders"
                        ...
                        <tbody>
                        <tr><td colspan="13" v-if="getStatusList.length===0"> 没有数据记录</td></tr>
                        <tr v-for="(item,index) in getStatusList">
                            <td>{{index + 1}}</td>
                            <td v-show="displayCols[0].display">{{item.orderId}}</td>
                            <td v-show="displayCols[1].display">
                                <!-- 在拒收这里需要弹窗来说明拒收原因； -->
                                <div v-if="item.statusName=='已拒收待审核'">
                                    <a href="javascript:;" @click="showRefuseReason(item.orderId)"> {{item.statusName ? item.statusName : ''}}</a>
                                </div>
                                 <!-- 在已取消 这里需要弹窗来说明取消原因； -->
                                <div v-else-if="item.statusName=='已取消待审核'">
                            ...
                            <td v-show="displayCols[2].display">{{item.orderPrice|formatPrice}}</td>
                            ...
                            <td v-show="displayCols[4].display">{{item.discount|formatPrice}}</td>
                            <td v-show="displayCols[5].display">{{item.orderTotalPrice|formatPrice}}</td>
                            <td v-show="displayCols[6].display">{{item.payAmount|formatPrice}}</td>
                            <td v-show="displayCols[7].display">{{item.orderCreateTime}}</td>
                            <td v-show="displayCols[8].display">{{item.updateTime}}</td>
                            <td v-show="displayCols[9].display">{{item.orderPaymentMethod | paymentType}}</td>
                            <td v-show="displayCols[10].display">{{item.orderPaymentTime}}</td>
                            <td v-show="displayCols[11].display">{{item.orderPaymentNum}}</td>
                            <td v-show="displayCols[12].display">{{item.orderReceiveTime}}</td>
                            <td v-show="displayCols[13].display">{{item.orderReceiverName}}</td>
                            <td v-show="displayCols[14].display">{{item.orderReceiverMobile}}</td>
                            <td></td>
                        </tr>
                        </tbody>
                    </table>
            ...
            <el-dialog title="物流信息" :visible.sync="seeLogicsVisible" size="tiny">
                ...
                <span slot="footer" class="dialog-footer">
                    <el-button type="primary" @click="autoReceiveGoods" v-if="isAuto && currentStatus == 3 || currentStatus == 19"
...
<script>
    ...
    import '../../../static/css/orderList.less';
    import Vue from 'vue';
    ...
    // 列表选项数据
    const colOptions = [
        { id: '0', name: '订单编号', isCheck: true, minWidth:'280px' }, { id: '1', name: '订货状态', isCheck: true,minWidth:'128px' }, { id: '2', name: '商品金额小计', isCheck: true,minWidth:'150px' },
        { id: '3', name: '运费金额', isCheck: true, minWidth:'180px' }, { id: '4', name: '折扣优惠', isCheck: true, minWidth:'110px' }, { id: '5', name: '合计金额', isCheck: true, minWidth:'120px'},
        { id: '6', name: '实付金额', isCheck: true,minWidth:'120px' }, { id: '7', name: '下单时间', isCheck: true, minWidth:'180px' }, { id: '8', name: '更新时间', isCheck: false,minWidth:'180px' },
        { id: '9', name: '支付类型', isCheck: false,minWidth:'80px' }, { id: '10', name: '支付时间', isCheck: false,minWidth:'180px' }, { id: '11', name: '支付流水号', isCheck: false,minWidth:'280px' },
        { id: '12', name: '收货时间', isCheck: false,minWidth:'180px' },{ id: '13', name: '收货人姓名', isCheck: false,minWidth:'120px' }, { id: '14', name: '收货人电话', isCheck: false,minWidth:'120px' },
    ];
...
//            this.token = window.localStorage.getItem('token');
            ...
            // 通过路由后面的参数，来确定，是正常点击进来，还是从订单确认详情页面 返回的；
            /*
            *  在点击按钮确认的时候，需要传入currentId,;
            *  从订单确认详情页回来的时候，需要传过来currentId和source，然后在进行调用，
            *  一次是需要请求数据渲染当前页面
            * 另一次需要打开确认模态框，然后获取数据
            * 在后面状态提交完毕后，重新刷新页面；置回当前路由状态；
             */
            ...
            //     }
            //     this.getData(() => {
            //         that.confirmOrder(that.currentId, that.currentStatus);
            //     });
            // }
            this.getData();
        },
        methods: {
            ...
            searchWithDate() {
                ...
                // this.orderList.orderReceiverMobile = this.orderList.orderReceiverName = this.basicSearch.receiver;
            ...
            confirmOrder(orderId, orderStatus) {
                ...
                this.$router.push({path:'/orderLogistics', query: {id : orderId,status: orderStatus, accountId: this.accountId, source:'list'}});
            },
...
//                let hostName = this.$globalData.host || 'http://10.100.50.59:8080';
                ...
                let accountId = this.accountId ? this.accountId : JSON.parse(sessionStorage.getItem('vuex')).user.account.accountId;
                window.location.href = this.baseUrl + '/order/export?pageNum=' + page + '&pageSize=' + this.pageSize + '&accountId=' + accountId + '&' + searchStr + '&token=' + this.token;
            },
            ...
            getData() {
                ...
                if(this.basicSearch.receiver) {
                     // this.orderList.orderReceiverMobile = this.orderList.orderReceiverName = this.basicSearch.receiver.trim();
                    this.orderList.nameOrPhone = this.basicSearch.receiver.trim();
                }
                ...
                if(this.$store.getters.returnFlag) {
                    ...
                    // filterParams 和 this.orderList指向同一内存地址；
            ...
            goToRefund(orderId,orderStatus){
                ...
                this.orderList.filterCheckedCols = this.checkedCols
                this.$store.dispatch('setPageInfo', this.orderList);
                this.$router.push({path:'/refundAudit', query: {id: orderId,status: orderStatus, source:'list',accountId: this.accountId}});
            },
            // 点击已拒收，查看拒收原因
            ...
            goToDetail(orderId, orderStatus){
                ...
                this.orderList.filterCheckedCols = this.checkedCols
                this.$store.dispatch('setPageInfo', this.orderList);
                this.$router.push({path:'/orderDetail', query: {id: orderId, status: orderStatus, accountId: this.accountId,source:'list'}});
            },
            ...
            resume() {
                ...
                this.changeSta = this.orderList.orderStatus;
                this.currentPage = this.orderList.pageNum;
                // 还原用户选择的列；
