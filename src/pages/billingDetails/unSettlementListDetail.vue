<template>
    <div class="unsettlementListDetail">
        ...
        <div class="unSettlementListDetailNumber">
            <div class="col-xs-2">
                <el-input
                    placeholder="订单编号"
                    icon="search"
            ...
            <div class="col-xs-3" style=" margin-top: 10px">
                <span>待结算总额:{{totalOrderRevenue | formatPrice}}</span>
            </div>
        </div>

        <div class="table-responsive">
            <table class="table table-bordered table-hover UnSettlementListDetail_table">
                <thead>
                <tr>
                    <th>序号</th>
                    <th>订单编号</th>
                ...
                <tr v-else v-for="(supplier,index) in orderData.list">
                    ...
                    <td>{{supplier.orderPrice | formatPrice}}</td>
                    ...
                    <td>{{supplier.orderDealerPrice  | formatPrice}}</td>
                    ...
                    <td>
                        ...
                        <div>
                            <a style="display: block"
                               @click="settlementListDetail_lookDetail(supplier.orderId)">查看明细</a>
                        </div>
                    </td>
                </tr>
                </tbody>
        ...
        <paging ref="page" :page-num="pageTotal" :current-page="currentPage" :pageSize="10"
                @change="change" v-on:changeR="changeRows" style="margin-left: 180px"></paging>
        <!--点击查看详情弹出的模态框-->
        <el-dialog :visible.sync="dialogTableVisible">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover UsettlementListDetail_table">
                    <thead>
                    <tr>
                        <th>商品图片</th>
                        <th>商品名称</th>
                        ...
                        <th>规格值</th>
                        <th>商品数量</th>
                        <th style="min-width: 100px">app订货价(¥)</th>
            ...
            <div class="settlementListDetail_divStyle">
                <paging ref="page" :page-num="dialogPageTotal" :current-page="dialogCurrentPage"
                        :dialogPageSize='10' @change="changeDialog"
                        v-on:changeR="changeDialogRows" :is-select="false" style="margin-left: 180px"></paging>
            </div>
        </el-dialog>
    </div>
</template>
...
<script>
    import '../../../static/css/unSettlementListDetail.css';
    ...
    export default {
        ...
        methods: {
            ...
            changeDialogRows(row) {
                ...
                this.settlementListDetail_lookDetail(this.orderDetailHeadData.orderId, 1);
            },
            ...
            goBarterDetail(detailsId){
                this.$router.push({path:'/barterListDetail', query: {detailsId: detailsId ,recordId: this.recordId,source: "unSettlementListDetail"}});
            },
            goReturnDetails(detailsId){
                this.$router.push({path:'/salesReturnAuditDetails', query: {detailsId: detailsId ,recordId: this.recordId,source: "unSettlementListDetail"}});
            },
            ...
            query() {
                ...
                api.getUnsettlementDetails(param).then(
                    (response) => {
             ...
             * @param orderId 订单编号
             */
            settlementListDetail_lookDetail(orderId, type) {
                ...
                api.getUnsettlementDetailsByOrderId(orderId, param).then(
                    (response) => {
                        let data = response.data.data;
                        if (!data) {
...
<style>
    .unsettlementListDetail .breadNav .el-breadcrumb__item__inner a {
        color: #48576a;
    }

    .unsettlementListDetail .breadNav .el-breadcrumb__item__inner a:hover {
        color: #20a0ff;
        cursor: pointer;
    }

    .UsettlementListDetail_table > tbody > tr > td {
        min-width: 90px;
        line-height: 40px;
        text-align: center;
        max-width:200px;
